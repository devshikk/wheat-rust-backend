"""
Inference engine — loads all trained models once at startup and exposes:
...
# ─── Model paths ──────────────────────────────────────────────────────────────
# MODELS_DIR can be overridden via environment variable for flexible deployment.
...
# ─── Load models at import time (once per worker) ────────────────────────────
...
# Grad-CAM sub-model: outputs (last_conv_features, global_avg_pool_features)
...
def _prepare_input(image_rgb: np.ndarray) -> np.ndarray:
    """Resize → denoise (CDAE) → preprocess for EfficientNet-B3."""
    img = cv2.resize(image_rgb, (224, 224)).astype(np.float32) / 255.0
    denoised = cdae.predict(img[np.newaxis, ...], verbose=0)   # (1, 224, 224, 3)
    return preprocess_input(denoised * 255.0)                   # (1, 224, 224, 3)
...
def predict(image_rgb: np.ndarray) -> dict:
    ...
    -------
    dict
        Keys: status (str), rust_type (str | None), confidence (float).
    """
